...
async function handleCallback(request, env) {
  ...
  if (!code || !state || !storedState || state !== storedState) {
    return redirectWithError(request, env, 'oauth_state_mismatch');
  }
  ...
  if (pathname === '/auth/logout' && request.method === 'POST') {
    return json(
      { ok: true },
      {
        status: 200,
        headers: {
          'Set-Cookie': clearCookie('legitcheck_session')
        }
      }
    );
  }

  if (pathname === '/auth/discord/start' && request.method === 'GET') {
    return handleStart(request, env);
  }

  if (pathname === '/auth/discord/callback' && request.method === 'GET') {
